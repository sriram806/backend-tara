...
export async function notificationAuthMiddleware(request: FastifyRequest, reply: FastifyReply) {
  const accessSecret = process.env.JWT_ACCESS_SECRET;
  if (!accessSecret) {
    return reply.code(500).send({
      success: false,
      error: { code: 'AUTH_CONFIG_MISSING', message: 'JWT access secret is not configured' }
    });
  }
